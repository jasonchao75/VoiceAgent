...
def get_files(root_dir, extension, exclude_dirs=None):
    ...
    for root, dirs, files in os.walk(root_dir):
        ...
        dirs[:] = [
            ...
            for d in dirs
            if not any(
                os.path.join(root, d).startswith(os.path.join(root_dir, excl))
...
def check_python_syntax():
    ...
    python_files = get_files(
        ".",
        ".py",
        exclude_dirs=["demos", "venv", ".venv", "frontend/node_modules", ".opencode/node_modules"],
    )
    errors = 0
...
def check_secrets():
    ...
    secret_patterns = [
        re.compile(
            r"(?:API_KEY|SECRET|TOKEN|PASSWORD)\s*(?:=|:)\s*['\"](?P<value>[a-zA-Z0-9_\-]{10,})['\"]",
            re.IGNORECASE,
        ),
            ...
            with open(f, encoding="utf-8") as file:
                ...
                for pattern in secret_patterns:
                    for match in pattern.finditer(content):
                        # Prefer the captured value so logs never expose the whole assignment.
                        val = (
                            match.group("value") if "value" in match.groupdict() else match.group(0)
                        )

                        # Check if it's an allowed placeholder
                        is_placeholder = (
                            any(
                                placeholder.lower() in val.lower()
                                for placeholder in allowed_placeholders
                            )
                            or val in allowed_placeholders
                        )
                        is_test_fixture = "tests" in Path(f).parts and val.lower().startswith(
                            ("test-", "inert-", "fake-", "secret-")
                        )

                        if not is_placeholder and not is_test_fixture:
