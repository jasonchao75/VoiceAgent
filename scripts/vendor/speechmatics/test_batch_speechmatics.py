...
def run_batch_test():
    ...
    if response.status_code != 201:
        logging.error(
            f"Failed to submit job. Status: {response.status_code}, Body: {response.text}"
        )
        return
    ...
    while True:
        ...
        if poll_response.status_code != 200:
            logging.error(
                f"Failed to poll job status. Status: {poll_response.status_code}"
            )
            return
    ...
    if transcript_response.status_code != 200:
        logging.error(
            f"Failed to get transcript. Status: {transcript_response.status_code}"
        )
        return
