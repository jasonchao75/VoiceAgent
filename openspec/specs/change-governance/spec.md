...
定义 VoiceAgent 使用主规格、Delta Specs 和 Change 归档管理行为变更的协作规则。
...
### Requirement: Legacy Change retention

OpenSpec 引入前的 Change 必须保留在 `docs/changes/archive/` 作为只读历史记录，不要求转换成 OpenSpec 格式；新建 Change 必须使用 `openspec/changes/`。

#### Scenario: Review a legacy Change

- **WHEN** Agent 需要追溯 OpenSpec 引入前的需求背景
- **THEN** Agent 可以读取 legacy archive，但不得把它当作当前系统行为的 Source of Truth
