...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const conversationalControls = document.createElement("fieldset");
  conversationalControls.id = "conversationalControls";
  conversationalControls.innerHTML = `
    <legend>Conversational controls</legend>
    <details class="voice-advanced" open>
      <summary>Advanced</summary>
      <label class="switch-row">
        <span><b>Allow callers to change speaking speed</b><small>Understands requests such as “speak faster”, “slow down”, and “return to normal”.</small></span>
        <input id="dynamicSpeedEnabled" type="checkbox">
      </label>
      <div class="voice-setting is-disabled" id="speedStepRow">
        <div><label for="speedStep">Adjustment per request</label><output id="speedStepValue">0.10</output></div>
        <input id="speedStep" type="range" min="0.05" max="0.25" step="0.05" value="0.10" disabled>
        <small>Subtle (0.05) ↔ Strong (0.25) · applies once per faster/slower request</small>
      </div>
      <p class="hint" id="dynamicSpeedHint">Session only · the Bot’s saved starting speed is unchanged.</p>
    </details>
  `;
  tuningFieldset.insertAdjacentElement("afterend", conversationalControls);

  const dynamicSpeedEnabled = document.querySelector("#dynamicSpeedEnabled");
  const speedStep = document.querySelector("#speedStep");
  const speedStepRow = document.querySelector("#speedStepRow");
  const dynamicSpeedHint = document.querySelector("#dynamicSpeedHint");

  function refreshDynamicSpeedFields() {
    const isV3 = providerSelect.value === "elevenlabs" && modelSelect.value === "eleven_v3";
    dynamicSpeedEnabled.disabled = isV3;
    if (isV3) dynamicSpeedEnabled.checked = false;
    speedStep.disabled = isV3 || !dynamicSpeedEnabled.checked;
    speedStepRow.classList.toggle("is-disabled", speedStep.disabled);
    dynamicSpeedHint.textContent = isV3
      ? "Eleven v3 does not support precise numeric Speed changes during a call."
      : "Session only · the Bot’s saved starting speed is unchanged.";
  }

  dynamicSpeedEnabled.addEventListener("change", refreshDynamicSpeedFields);
  speedStep.addEventListener("input", () => {
    document.querySelector("#speedStepValue").value = Number(speedStep.value).toFixed(2);
  });
  providerSelect.addEventListener("change", refreshDynamicSpeedFields);
  modelSelect.addEventListener("change", refreshDynamicSpeedFields);
  refreshDynamicSpeedFields();
});
