...
- [x] Paused/partially-failed batches with a preliminary report expose “Use current results”; the confirmation discloses preserved successes, excluded work, immutable partial coverage and zero ASR/LLM calls, with no horizontal overflow at desktop or narrow viewport
...
- [x] A terminal plan remains `completed` when provider attempts are unavailable; evidence-insufficient Cases are shown as excluded coverage rather than batch failure
- [x] Scoped retry confirmation discloses eligible/skipped counts, unknown usage and maximum additional cost; the consumed versioned plan is the dispatch whitelist
- [x] Historical report conversation and audio requests include `batch_id` and continue reading the frozen dataset after the active source changes
- [x] Dashboard cards render active-source dataset ID, selected-report batch ID/denominator and global-Benchmark totals as three non-interchangeable scopes
...
| Stability closure retry + completed semantics | existing frozen batch action/report patterns / PD-080–PD-082 | production route and DOM at 1440×1000 and 1024×1000 | targeted | — | completed batch remains completed; scoped retry shows plan counts, unknown usage and cost ceiling; exact plan hash is posted | native confirmation and both supported viewports pass | deterministic intercepted APIs; no provider call or production mutation | ready for independent re-review |
| Frozen historical source + dashboard scopes | existing frozen report drawer and four-card dashboard / PD-080–PD-082 | production route and DOM at 1440×1000 and 1024×1000 | targeted | — | report drawer/audio carry batch ID; active source, selected report and global Benchmark identities/denominators render independently | both supported viewports pass | fixture plus isolated formal API route; no baseline geometry change | ready for independent re-review |
