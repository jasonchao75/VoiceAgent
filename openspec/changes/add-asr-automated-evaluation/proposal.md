...
- 每家评测 ASR 对每个命中的完整对话只提交一次显式开启说话人分离的异步文件转写；系统结合历史文本和事件顺序映射匿名说话人，并要求至少两家完整录音 ASR 对目标用户事件的时间区间形成共识。Excel `time (s)` 不参与定位，纯用户 WAV 只校验并微调共识边缘；最终单句切片再分别提交给各评测 ASR。
