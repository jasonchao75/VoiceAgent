...
- 每家评测 ASR 对每个命中的完整对话只提交一次显式开启说话人分离的异步文件转写。第 4 步“音频与证据对齐”先以纯用户 WAV 的语音岛冻结 Case 边界，再按历史事件顺序、数字/拼写归一化、相邻机器人轮次和多 provider 文本一致性做确定性单调匹配；历史 `time (s)` 不参与强约束，文本和 ASR turn 时间不得移动音频边界。一个语音岛对应多个相邻历史事件时合并为一个 Case 并保留全部 event ID。
- 只有 ASR Turn、历史事件与语音岛仍无法唯一对应的歧义 Case 才调用批次冻结的辅助 LLM。模型只能从真实候选 ID 中选择；输出经确定性校验后才能使用，否则进入人工复核。生成的纯用户切片只用于回听、人工复核和 Benchmark，不再提交评测 ASR。
- 第二轮 LLM 综合线上历史转写、完整对话、质检上下文、标签字典和已投影的各家完整录音 ASR turn 文本，输出 `Good Case`、`Bad Case` 或 `需人工复核`，不使用置信度阈值控制入库。完整历史文本保留；完整录音 ASR 证据只携带目标 turns 和直接相邻 turns，单通仍超限时在发送前按稳定 Case 子集拆组。
...
- Audio-first 发现的历史 Turn 异常先按异常组进入独立人工复核；页面展示原始 event ID、建议 Case 映射、音频和 provider 证据，复核人员明确确认或驳回。报告只把人工确认的异常组与受影响 Turn 行数作为确定结论，同时披露待复核数量和覆盖率；该数据质量指标不进入 ASR 错误率，也不直接创建 Benchmark。
...
- `PRD.html V1.17` 与 `prototypes/index.html` V1.20（SHA-256 `a37220ea1a24e7a538cfdf8677612fe0d29e00a6fca46063e01e316703f91e62`）是当前唯一冻结基线；V1.19 仅保留为已被 PD-076 取代的历史原型。User Gate 1 已续订，正式实现仍需后续明确开发指令并完成 Engineering Checkpoint A/B/C。
...
- Benchmark 以 `(conversation_id, event_id)` 全库唯一；后续批次的相同结果幂等复用，冲突结果直接丢弃新候选，不覆盖、不修订且不进入人工冲突复核（PD-073、PD-075）。
