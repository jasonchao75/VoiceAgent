...
- **显式调用（角色切换，推荐）**：对于明确的阶段性任务（例如调研结束进入开发阶段），建议在对话开始时通过**身份设定**进行显式切换（如：“当前角色：Integration-Developer。请开始搭建 Pipecat 基础管道”）。这种方式能让 Agent 更聚焦于当前阶段的核心职责（参考第5节），避免在写代码时被调研思维干扰。
